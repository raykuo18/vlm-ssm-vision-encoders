...
class ScoreConfig:
    ...
    model_id: str = DEFAULT_MODEL_ID                # Model ID to load and run (instance of `model_family`)
    ...
    allow_partial_results: bool = False            # Allow scoring capped debug runs with fewer than expected examples
...
def score_after_parse(cfg):
    ...
    if not cfg.allow_partial_results:
        assert (
    ...
    elif len(full_results) != cfg.dataset.expected_examples:
        overwatch.warning(
            f"Scoring partial debug results for `{cfg.model_id}`: "
            f"found {len(full_results)} / {cfg.dataset.expected_examples} examples."
        )
    ...
    metrics = {
        ...
        "n_examples": len(full_results) if cfg.allow_partial_results else cfg.dataset.expected_examples,
        "model": cfg.model_id,
