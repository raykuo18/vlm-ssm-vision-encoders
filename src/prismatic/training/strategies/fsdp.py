...
class FSDPStrategy(TrainingStrategy):
    def __init__(
        ...
        checkpoint_stage_dir: Optional[Path] = None,
        ...
        self.checkpoint_stage_dir = Path(checkpoint_stage_dir).expanduser() if checkpoint_stage_dir else None
        self._checkpoint_executor: Optional[ThreadPoolExecutor] = None
        self._pending_checkpoint_futures: List[Future] = []
        self._checkpoint_stage_enabled = (
            self.enable_checkpointing and overwatch.is_rank_zero() and self.checkpoint_stage_dir is not None
        )
        if self._checkpoint_stage_enabled:
            self.checkpoint_stage_dir.mkdir(parents=True, exist_ok=True)
            self._checkpoint_executor = ThreadPoolExecutor(max_workers=1, thread_name_prefix="ckpt-upload")

    def _point_latest_checkpoint(self, checkpoint_path: Path, latest_checkpoint: Path) -> None:
        try:
            if latest_checkpoint.exists() or latest_checkpoint.is_symlink():
                latest_checkpoint.unlink()
            os.link(checkpoint_path, latest_checkpoint)
        except OSError:
            shutil.copy(checkpoint_path, latest_checkpoint)

    def _finalize_checkpoint_from_stage(
        self,
        stage_path: Path,
        ...
        shutil.copy(stage_path, checkpoint_path)
        duration = time.time() - start_time
        self._point_latest_checkpoint(checkpoint_path, latest_checkpoint)
        stage_path.unlink(missing_ok=True)
        overwatch.info(
            f"[Checkpoint] Async copy complete: {checkpoint_path.name} | Size = {payload_size / 1e9:.2f} GB | Time = {duration:.1f}s"
        )

    def _prune_completed_checkpoint_uploads(self) -> None:
        if not self._pending_checkpoint_futures:
            return
        active: List[Future] = []
        for future in self._pending_checkpoint_futures:
            if future.done():
                future.result()
            else:
                active.append(future)
        self._pending_checkpoint_futures = active

    def _wait_for_pending_checkpoint_uploads(self) -> None:
        if not self._pending_checkpoint_futures:
            return
        for future in self._pending_checkpoint_futures:
            future.result()
        self._pending_checkpoint_futures.clear()

    def wait_for_pending_io(self) -> None:
        if self._checkpoint_stage_enabled:
            self._wait_for_pending_checkpoint_uploads()
            if self._checkpoint_executor is not None:
                self._checkpoint_executor.shutdown(wait=True)
                self._checkpoint_executor = None
            if self.checkpoint_stage_dir.exists():
                for leftover in self.checkpoint_stage_dir.glob("*"):
                    try:
                        ...
                        continue
                try:
                    self.checkpoint_stage_dir.rmdir()
                except OSError:
                    pass
    ...
    def save_checkpoint(
        ...
        with FSDP.state_dict_type(self.vlm, self.fsdp_state_dict_type, self.fsdp_save_policy):
            ...
            if overwatch.is_rank_zero():
                self._prune_completed_checkpoint_uploads()
                ...
                payload = {"model": model_state_dicts}

                if self._checkpoint_stage_enabled:
                    stage_filename = f"{checkpoint_path.stem}-{uuid.uuid4().hex}.pt"
                    stage_path = self.checkpoint_stage_dir / stage_filename
                    torch.save(payload, stage_path)
                    payload_size = stage_path.stat().st_size
                    start_time = time.time()

                    if self._checkpoint_executor is not None:
                        overwatch.info(
                            f"[Checkpoint] Async copy queued: {checkpoint_path.name} | Size = {payload_size / 1e9:.2f} GB"
                        )
                        future = self._checkpoint_executor.submit(
                            self._finalize_checkpoint_from_stage,
                            stage_path,
                            checkpoint_path,
                            latest_checkpoint,
                            payload_size,
                        ...
                        self._pending_checkpoint_futures.append(future)
                    else:
                        self._finalize_checkpoint_from_stage(
                            stage_path, checkpoint_path, latest_checkpoint, payload_size, start_time
                        )
                else:
                    torch.save(payload, checkpoint_path)
                    payload_size = checkpoint_path.stat().st_size
                    overwatch.info(
                        f"[Checkpoint] Saved {checkpoint_path.name} | Size = {payload_size / 1e9:.2f} GB (no staging)"
                    )
                    self._point_latest_checkpoint(checkpoint_path, latest_checkpoint)
    ...
    def run_setup(self, run_dir: Path, n_train_examples: int) -> None:
        ...
        if world_size <= 1 and self.fsdp_sharding_strategy in (
            ShardingStrategy._HYBRID_SHARD_ZERO2,
            ShardingStrategy.HYBRID_SHARD,
        ):
            overwatch.info("FSDP world_size=1; switching sharding_strategy to NO_SHARD for local tests.")
        ...
        # TODO: Customize for vision backbones
