...
class AlignDataset(Dataset[Dict[str, torch.Tensor]]):
    ...
    def get_max_text_tokens(self, multimodal_only: bool = True) -> int:
        cache_key = "_max_text_tokens_multimodal" if multimodal_only else "_max_text_tokens_all"
        ...
        for example in self.examples:
            if multimodal_only and "image" not in example:
                continue
            conversation = example["conversations"]
            if not conversation:
                continue
            caption = self.prompt_template.format(caption=conversation[-1]["value"].strip())
            input_ids = self.tokenizer(caption, add_special_tokens=True).input_ids
            max_len = max(max_len, len(input_ids))

        setattr(self, cache_key, max_len)
        return max_len
...
class FinetuneDataset(Dataset[Dict[str, torch.Tensor]]):
    ...
    def get_max_text_tokens(self, multimodal_only: bool = True) -> int:
        ...
        for example in self.examples:
            ...
            prompt_builder = self.prompt_builder_fn(model_family="prismatic")
            input_ids: List[int] = []
            for turn_idx, turn in enumerate(conversation):
                msg = prompt_builder.add_turn(turn["from"], turn["value"])
                if isinstance(self.tokenizer, LlamaTokenizerFast):
